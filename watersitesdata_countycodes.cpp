...
const QString WaterSitesDataCountyCodes::FORMAT = "countycodes,1.0";

/**
*/
QString WaterSitesDataCountyCodes::getFormat()
{
    return FORMAT;
}
...
/**
*/
void WaterSitesDataCountyCodes::extractData( WaterSitesEngine *engine, const QString &request, QByteArray &bytes )
{
    QDomDocument doc;
    QString errorMsg;
    ...
    if (doc.setContent(QString(bytes), &errorMsg, &errorLine, &errorColumn))
    {
        ...
        if (document.tagName() == "countycodes")
        {
            ...
            engine->setEngineData(request, I18N_NOOP(WaterSitesEngine::PREFIX_XML + "error_msg"), "XML Error: format is not countycodes.");
...
void WaterSitesDataCountyCodes::extractCodes( WaterSitesEngine *engine, const QString &request, QDomElement *document )
{
    QString filter = "all";
    if (request.contains("?"))
    {
        QString option = request.split("?").at(1);
        if (option != "") filter = option.toLower();
    }
    ...
    for (int j=0; j<num_codes; j++)
    {
        ...
        QString stateCode = code.attribute("state", "-1");
 
        if (stateCode == filter || filter == "all")
        {
            codeHash = QHash<QString,QVariant>();
            codeHash["name"] = code.attribute("name", "-1");
            codeHash["statecode"] = stateCode;
            codeHash["countycode"] = code.attribute("county", "-1");

            engine->setEngineData(request, WaterSitesEngine::PREFIX_COUNTYCODE + codeHash["statecode"].toString() + codeHash["countycode"].toString(), codeHash);
            codeCount += 1;
        }
    }

    engine->setEngineData(request, I18N_NOOP(WaterSitesEngine::PREFIX_COUNTYCODE + "count"), codeCount);
}
